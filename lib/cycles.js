...
export function computeCycleStats(attendancesAsc, purchases = []) {
  ...
  // Per-ticket usage ("3/10") — keyed by ticket id, in the same oldest-first
  // consumption order as everything else here.
  const ticketUsage = {};
  for (const ticket of ticketsAsc) {
    const usedFromThisTicket = Math.min(ticket.lessons_count, Math.max(0, totalAttended - consumed));
    ticketUsage[ticket.id] = usedFromThisTicket;
...
// The single client-list/attendance-list pill, derived from computeCycleStats
// — kept in one place so every screen that shows it agrees. Ticket size is
// never shown (it's always CYCLE_LENGTH and not adjustable), just a number
// and what it means:
//  - 'overdraft': attended beyond every ticket's capacity — the urgent,
//    red case. Value is the overdraft count (in practice 1-3).
//  - 'progress': partway through her current ticket, nothing owed — value
//    is how many lessons she's used from it so far (1 to CYCLE_LENGTH-1).
//  - 'noTicket': her last ticket is fully drawn down and she hasn't
//    attended since (so no overdraft yet either) — she needs a new one,
//    but there's no number to show.
//  - 'empty': nothing to show — either no ticket purchased at all, or a
//    brand-new ticket with nothing attended from it yet.
export function pillStateFromStats(stats) {
  if (stats.overdraftCount > 0) return { kind: 'overdraft', value: stats.overdraftCount };
  if (!stats.lastTicket) return { kind: 'empty' };
  const used = stats.ticketUsage[stats.lastTicket.id] ?? 0;
  if (used === 0) return { kind: 'empty' };
  if (used >= stats.lastTicket.lessons_count) return { kind: 'noTicket' };
  return { kind: 'progress', value: used };
}

// Urgency order for "sort by debt": every overdraft client first (highest
// overdraft first), then everyone mid-ticket (highest usage first), then
// "needs a new ticket", then everyone with nothing to flag.
export function pillSortRank(pill) {
  if (!pill) return 0;
  if (pill.kind === 'overdraft') return 2_000_000 + pill.value;
  if (pill.kind === 'progress') return 1_000_000 + pill.value;
  if (pill.kind === 'noTicket') return 500_000;
  return 0;
}
